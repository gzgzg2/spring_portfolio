...
function loadPage(page){
	...
	$.ajax({
		url : "${pageContext.request.contextPath}/AdminBookAJAX/admin_InnManage/<%=writePages%>/"+ page,
					type : "GET",
					cache : false,
	...
	function search(search_page){
		...
		// AJAX
		$.ajax({
			url : "${pageContext.request.contextPath}/AdminBookAJAX/admin_InnManage/" + search_option + "/" + keyword + "/<%=writePages%>/"+search_page,
			type : "GET",
			cache : false,
			success : function(data, status){
				if(status == "success"){
